...
<script>
  ...
  import {getGoods2,sxGoods} from "@/api/goods";
  export default {
    components: {
      ElButton,
      ElCol
    ...
    methods: {
      ...
      handleEdit(index, row) {
        ...
        this.$router.push({name: 'gsupdata', params: row});
      },
      ...
      handleDownload() {
        require.ensure([], () => {
          ...
          const filterVal = ['name', 'date', 'name', 'price', 'sort']
          const list = this.goods
          const data = this.formatJson(filterVal, list)
          export_json_to_excel(tHeader, data, '服务类商品')
